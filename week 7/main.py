...
# member query api
# /api/member
@app.get('/api/member/{target_id}')
async def get_member(target_id:Annotated[int,None],request:Request):
    member = request.session.get('member')
    if not member:
    ...
    #如果找不到這個id，就部寫入query_log，直接回傳no data
    if not member_data:
        cursor.close()
        con.close()
        return {"data": None}

 # task4 tracking member queries
 #如果searcher id=target id
    
    if member['id']!=target_id:
        cursor.execute('INSERT INTO query_log(target_id,searcher_id) VALUES (%s,%s)',[target_id,member['id']])
        con.commit()
    cursor.close()
    con.close()
...
#show出前10個searcher
@app.get('/api/query')
async def show_searchers(request:Request):
    member = request.session.get('member')
    if not member:
        return {"error": "not logged in"}
    ...
    query="""
        ...
        LIMIT 10;
"""
    cursor.execute(query,(current_member_id,)) 
    #自己登入的id就是target_id
...
async def patch_member(request:Request,body=Body(None)):
    #確認是否登入
    member=request.session.get('member')
    if not member:
        return {'error':True, "message": "未登入，無法修改"}
    ...
    cursor.execute(query,(new_name,currentUserId))
    con.commit()
    cursor.close()
    con.close()
    #更新成功
